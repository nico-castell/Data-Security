...
char NormalizeASCII(const char& character) { return 33 + (character % 94); }

// Add the numerical values of two characters, normalize for the ASCII table
char AddChars(const char& base_char, const char& key_char) { return NormalizeASCII((base_char + key_char) % 127); }
...
char MultiplyChars(const char& base_char, const char& key_char) { return NormalizeASCII((base_char * key_char) % 127); }
